...
export default function ContasReceberPage() {
  ...
  async function marcarComoRecebida(contaId: number) {
    const confirmar = confirm("Deseja marcar esta conta como recebida?")
    ...
    const resposta = await fetch(
      `http://127.0.0.1:8000/contas-receber/${contaId}/receber`,
      {
        method: "PATCH",
        headers: {
  ...
  async function cancelarConta(contaId: number) {
    ...
    const resposta = await fetch(
      `http://127.0.0.1:8000/contas-receber/${contaId}/cancelar`,
      {
        method: "PATCH",
      }
    )
  ...
  const contasFiltradas = useMemo(() => {
    return contas.filter((conta) => {
      ...
      const combinaBusca =
        ...
        (conta.cliente_nome || "").toLowerCase().includes(textoBusca) ||
  ...
  const totalEmAberto = contas
    .filter((conta) => conta.status === "Em aberto" || conta.status === "Atrasado")
  ...
  const quantidadeEmAberto = contas.filter(
    (conta) => conta.status === "Em aberto" || conta.status === "Atrasado"
  ).length
        ...
        <div className="flex items-center justify-between mb-8 gap-6">
          <div>
            ...
            <p className="text-gray-600">
              Controle valores pendentes, clientes, vencimentos e recebimentos.
            </p>
          </div>

          <Link
            href="/contas-receber/novo"
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-gray-500 mt-1">
              Acompanhe vencimentos, valores, status e clientes vinculados.
            </p>
          </div>
                    ...
                    >
                      <td className="p-4">
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          Recebimento: {conta.forma_recebimento || "-"}
                        </div>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex flex-wrap gap-2">
                          <Link
                            href={`/contas-receber/editar/${conta.id}`}
