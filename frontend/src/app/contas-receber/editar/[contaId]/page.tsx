...
export default function EditarContaReceberPage() {
  ...
  async function buscarConta() {
    const resposta = await fetch(
      `http://127.0.0.1:8000/contas-receber/${contaId}`
    )

    const dados: ContaReceber = await resposta.json()
    ...
    setClienteId(dados.cliente_id ? String(dados.cliente_id) : "")
    setDescricao(dados.descricao || "")
    setCategoria(dados.categoria || "")
    setValor(String(dados.valor || ""))
    setDataVencimento(normalizarData(dados.data_vencimento))
    setDataRecebimento(normalizarData(dados.data_recebimento))
    setFormaRecebimento(dados.forma_recebimento || "")
  ...
  async function salvarConta(event: React.FormEvent) {
    ...
    if (dados.mensagem === "Conta a receber atualizada com sucesso") {
      ...
      <div className="max-w-4xl mx-auto pl-24">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Atualize os dados do cliente, vencimento, valor e recebimento.
          </p>
        </div>
